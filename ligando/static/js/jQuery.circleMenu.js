...
(function ($, window, document, undefined) {
    ...
    function CircleMenu(element, options) {
        ...
        this.pos = this.element.children('li:first-child').position();
        this.pos = this.element.children('li:first-child').position();
        this.init();
        this.hook();
    }
    ...
    CircleMenu.prototype.init = function () {
        ...
        self.item_mod = Math.ceil(Math.sqrt(self.item_count));
        self._step = (self.options.angle.end - self.options.angle.start) / (self.item_count - 1);
        self.menu_items.each(function (index) {
            var $item = $(this);
            var radius = self.options.item_diameter;
            //angle = (self.options.angle.start + (self._step * index)) * (Math.PI/180),
            // check if number is even
            //if(self.item_mod %2 !=0){
            //y = self.pos.top+ ((self.options.item_diameter*(index % self.item_mod))-(self.options.item_diameter * self.item_mod%2));
            //x = self.pos.left+ ((self.options.item_diameter*((index)-(index % self.item_mod)*self.item_mod))-(self.options.item_diameter * self.item_mod%2));

            y = self.pos.top + ((radius * (index % self.item_mod)) - (radius * self.item_mod % 2));
            console.log((radius * ((index) - (index % self.item_mod) * self.item_mod)));
            console.log((radius * self.item_mod % 2));
            x = self.pos.left + ((radius * ((index) - (index % self.item_mod) * self.item_mod)) - (radius * self.item_mod % 2));
            //console.log(self.pos.top);
            //console.log(self.pos.left);

            //}
            //x = pos.left+10; // +  Math.round(self.options.circle_radius * Math.cos(angle)),
            //y = pos.top+10;  //+ Math.round(self.options.circle_radius * Math.sin(angle));
    ...
    CircleMenu.prototype.close = function (immediate) {
        var self = this,
            ...
            do_animation = function do_animation() {
                ...
                set.each(function (index) {
                    var $item = $(this);
                    var pos = self.element.children('li:first-child').position();
                    self._timeouts.push(setTimeout(function () {
